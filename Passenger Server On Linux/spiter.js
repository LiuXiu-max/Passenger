...
for(var i=1;i<=20;i++){
    ...
    https.get(url,function(res){
        ...
        res.on('end',function(i){
            ...
            let attrlist=['id','name','authorId','viewCount','likeCount','commentCount','publishTime','picUrl','authorName','authorHeadImg','summary'];
            let myclass=filter.fliter_class_attr(JSON.parse(json).data.rows,attrlist);
            console.log('数据清洗完毕，开始导入数据库');
            for(var i=0;i<myclass.length;i++){
                dosql.InsertMysql('insert into trips set ?',myclass[i],'Passenger');
            }
            console.log('数据导入完毕');
        });
    });

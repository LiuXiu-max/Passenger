...
app.get('/api/imageurls/:tripId',function(req,res){
        let startPath=__dirname+'\\tripimages\\'+req.params.tripId;
        let result=[];
        if(fs.existsSync(startPath)){
                ...
                if(stats.isFile()) {
                        let imageurl='http://localhost:3000/api/getimage/'+req.params.tripId+'/'+val;
...
app.get('/api/getimage/:tripId/:tripName',function(req,res){
        let imagepath=__dirname+'\\tripimages\\'+req.params.tripId+'\\'+req.params.tripName;
...
app.get('/api/testtripslist/:userid/:start/:end',function(req,res,next){
        let conn=consql.ConMysql('test');
        ...
        for(var i=0;i<result.length;i++){
                console.log('before'+result[i].id)
                opredis.ifLike(req.params.userid,result[i].id).then((res)=>{    
                        //console.log('after'+result[i].id)
                        console.log('after');
                        console.log(res);
                        if(res){
...
app.post('/api/getrecommendlist',async function(req,res){
        ...
        ,async function(error,result,fields){
                if(error){
                        ...
                        client=opredis.newClient();
                        for(var i=0;i<req.body.end-req.body.start;i++){
...
app.get('/api/historylist/:id/:start/:end',async function(req,res){
        ...
        ,async function(error,result,fields){
                if(error){
                        ...
                        console.log('get historytripslist success');
